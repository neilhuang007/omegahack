...
import static me.alpha432.oyvey.features.command.Command.sendMessage;
...
public class ClickGui
    ...
    String currentconfig = new String("config");
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                ...
                sendMessage("Prefix set to " + ChatFormatting.DARK_GRAY + OyVey.commandManager.getPrefix());
            }
    ...
    public void onDisable() {
        if (mc.currentScreen != null && mc.currentScreen instanceof OyVeyGui) {
            mc.displayGuiScreen(null);
        }
        OyVey.configManager.saveConfig(currentconfig);
    }
